	...
	uint8_t* output, size_t outputLen, uint8_t* key/*256 bits key*/)
{
	...
	{
		cipher(inputBuf + i, output + i, w /* expanded key */);
	}
...
int main2(int argc, char *argv[]) {
	...
	*/
	...
	cipher(in1 /* in */, out /* out */, w /* expanded key */);
	printf("out:\n");
	for (i = 0; i < 4; i++) {
	...
	cipher(in2 /* in */, out /* out */, w /* expanded key */);
	printf("out:\n");
	for (i = 0; i < 4; i++) {
		printf("%x %x %x %x ", out[4 * i + 0], out[4 * i + 1], out[4 * i + 2], out[4 * i + 3]);
	}
	...
	aesEncrypt(in1, 16, out, 16, key);
	for (i = 0; i < 4; i++) {
		printf("%x %x %x %x ", out[4 * i + 0], out[4 * i + 1], out[4 * i + 2], out[4 * i + 3]);
	}
	printf("\n");

	aesEncrypt(in2, 16, out, 16, key);
	for (i = 0; i < 4; i++) {
		printf("%x %x %x %x ", out[4 * i + 0], out[4 * i + 1], out[4 * i + 2], out[4 * i + 3]);
	}
	printf("\n");

	aesEncrypt(in3, 17, out, 32, key);
	for (i = 0; i < 4; i++) {
		printf("%x %x %x %x ", out[4 * i + 0], out[4 * i + 1], out[4 * i + 2], out[4 * i + 3]);
	}
	printf("\n");
